...
def get_user(user_id: int) -> dict:
    ...
    if conn:
        try:
            c = conn.cursor()
            c.execute(
                "SELECT profile, consent, consent_time, premium_expiry, ban_type, ban_expiry "
        ...
        if conn:
            try:
                ...
                c.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                """, (
                    user_id, partner_id, int(time.time()),
...
def admin_info(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if conn:
            try:
                c = conn.cursor()
                c.execute("SELECT COUNT(*) FROM reports WHERE reported_id = %s", (target_id,))
                count = c.fetchone()[0]
...
def main() -> None:
    ...
    logger.info("Starting ChatDude bot...")
